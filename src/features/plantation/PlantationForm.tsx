...
function initialValues(cohort?: PlantationCohort) {
  return { cropId: cohort?.cropId ?? "", farmAreaId: cohort?.farmAreaId ?? "", quantity: cohort ? String(cohort.quantity) : "", plantingDate: cohort?.plantingDate ?? todayInKolkata(), notes: cohort?.notes ?? "" };
}
...
export function PlantationForm({ cohort, onSaved }: { cohort?: PlantationCohort; onSaved: () => void }) {
  ...
  const mutation = cohort ? update : create;
