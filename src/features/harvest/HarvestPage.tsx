...
export function HarvestPage() {
  ...
  return <section className="harvest-page">
    ...
    <Card className="harvest-recent"><CardHeader><div><span>Latest field entries</span><CardTitle>Recent harvest records</CardTitle></div><strong>{harvests.data?.length ?? 0} shown</strong></CardHeader><CardContent>{harvests.data?.length ? <ol>{harvests.data.map((harvest) => <li key={harvest.id}><div className="harvest-recent__date"><span>{dateLabel(harvest)}</span><small>{harvest.buyer ?? "Buyer not recorded"}</small></div><div className="harvest-recent__crop"><strong>{harvest.cropName}</strong>{!harvest.cropActive ? <small>Inactive historical crop</small> : null}</div><div className="harvest-recent__yield"><span>{formatWeight(Number(harvest.netWeightKg ?? 0))}</span><b>{formatINR(harvest.actualRevenuePaise ?? harvest.calculatedRevenuePaise ?? 0)}</b></div>{canWrite ? <div className="harvest-recent__actions"><Button aria-label={`Edit ${harvest.cropName} harvest ${harvest.id}`} onClick={() => openEdit(harvest)} size="icon" variant="ghost"><Edit3 aria-hidden="true" size={16} /></Button><Button aria-label={`Delete ${harvest.cropName} harvest ${harvest.id}`} onClick={() => { remove.reset(); setDeleting(harvest); }} size="icon" variant="ghost"><Trash2 aria-hidden="true" size={16} /></Button></div> : null}</li>)}</ol> : <EmptyState action={canWrite ? <Button onClick={openCreate}>Record the first harvest</Button> : undefined} description="Adjust the filters or record a field sale to begin this ledger." title="No harvest records found" />}</CardContent></Card>
