...
export function HarvestForm({ harvest, onSaved }: { harvest?: Harvest; onSaved: () => void }) {
  ...
  return <form className="harvest-form" noValidate onSubmit={submit}>
    <div className="harvest-form__grid">
      <div className="field"><label className="field__label" htmlFor="harvest-crop">Crop</label><select aria-describedby={errors.cropId ? "harvest-crop-error" : undefined} aria-invalid={Boolean(errors.cropId)} className="field__input" id="harvest-crop" onChange={(event) => set("cropId", event.target.value)} value={values.cropId}><option value="">Select crop</option>{options.map((crop) => <option key={crop.id} value={crop.id}>{crop.name}{crop.active ? "" : " (inactive)"}</option>)}</select>{errors.cropId ? <span className="field__error" id="harvest-crop-error">{errors.cropId}</span> : null}</div>
