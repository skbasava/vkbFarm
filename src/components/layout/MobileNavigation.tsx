...
type MobileNavigationProps = { canQuickAdd: boolean; onQuickAdd: (trigger: HTMLButtonElement) => void };
type NavigationGroup = "farm" | "more";

export function MobileNavigation({ canQuickAdd, onQuickAdd }: MobileNavigationProps) {
  ...
  return <nav aria-label="Mobile navigation" className={`mobile-nav${canQuickAdd ? "" : " mobile-nav--readonly"}`}>
    <NavLink aria-label="Home" end to="/dashboard"><House aria-hidden="true" size={19} /><span>Home</span></NavLink>
    <NavLink end to="/expenses"><ReceiptText aria-hidden="true" size={19} /><span>Expenses</span></NavLink>
    {canQuickAdd ? <button aria-label="Add record" className="mobile-nav__quick" onClick={(event) => onQuickAdd(event.currentTarget)}><span><Sprout aria-hidden="true" size={23} /></span><b>Quick add</b></button> : null}
